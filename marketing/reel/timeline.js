 ...
 * speaking rate; when the day's text needs more time than a scene's design
 ...
 *
 * The scene cuts and the 104 BPM beat grid lock the synthesized BGM, the
 * SFX, and the visual entrances to the same grid without any runtime
 * beat-detection.
 */
(function (g) {
  ...
  const DESIGN_BEATS = [
    ...
    ['b5', 11.2, 14.4],  // answer settled EN+JP — stretched to fit the TTS
    ['b6', 14.4, 17.0],  // patient reply EN — stretched to fit the TTS
  ...
  function makeReel(BEATS) {
    ...
    return { BEATS, TOTAL, BPM, beatTimes, beat, gridTimes, wordTimes, withBeats: makeReel };
  }
