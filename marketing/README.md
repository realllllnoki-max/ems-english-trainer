...
- **BGM** — ビートグリッド(既定104BPM)に同期したキック・ハイハット・ベースを
  コードで合成(ライセンスフリー・毎回同一)
...
TikTokの**トレンド音源を使いたい日**は、`--silent` で無音版を生成して
投稿時にアプリ内で音源を付けてください:

```bash
node generate.mjs --silent   # 無音版(トレンド音源を後付けする用)
```
