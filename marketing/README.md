...
EMS English Trainer の集客用ショート動画(1080×1920・約23秒)を、
...
| 17.0–19.4 | 患者の返答 | 日本語訳+フレームワーク |
| 19.4–21.4 | CTA | アプリ紹介+「プロフィールのリンクから」 |
| 21.4–22.8 | ループブリッジ | 「言えたら🔥をコメント」→白フラッシュで先頭へ |

尺は約23秒・60fps。シーン切替は背景色ごと変わるハードカット、
表示中も常に微ズーム(Ken Burns)し続けます。動きは GSAP のイージング
(back/expo/sine など)で駆動し、単語のポップは104BPMのビートグリッドに吸着します。
